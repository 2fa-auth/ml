...
class CNNBlock(nn.Module):
  def __init__(self, in_channels, out_channels, **kwargs):
    ...
    self.leakyrelyu = nn.LeakyReLU(0.1)

  def forward(self, x):
    return self.leakyrelyu(self.batchnorm(self.conv(x)))
...
class Yolov1(nn.Module):
  ...
  def _create_fcs(self, split_size, num_boxes, num_classes):
    ...
    return nn.Sequential(
      ...
      nn.Dropout(0.),
      nn.LeakyReLU(0.1),
      nn.Linear(496, S * S * (C + B * 5)), # число 5 это чисел на рамку (x, y, w, h, confidence)
    )
